...
We also wanted to balance building with buying, keeping in mind our slim finances. While its impossible to truly start from scratch\footnote{``If you want to make an apple pie from scratch, you must first create the universe'' - Dr. Carl Sagan \cite{saganquote}}, we tried to build whatever we could given the 16th June competition deadline. 

% footnote is broken (breaks from bottom of page one onto bottom of page two), something about the documentclass needs to be changed so it doesn't assume pages are next to each other (book-like)?
...
% -- amanda will finish this by mon noon: 
